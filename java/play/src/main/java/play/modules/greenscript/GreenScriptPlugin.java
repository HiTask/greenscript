...
public class GreenScriptPlugin extends PlayPlugin {
	...
	private static Properties defProps_;
	static {
		...
		// operation switches
		defProps_.setProperty("greenscript.minimize",
				Play.mode == Mode.PROD ? "true" : "false");
		defProps_.setProperty("greenscript.compress",
				Play.mode == Mode.PROD ? "true" : "false");
		defProps_.setProperty("greenscript.cache",
				Play.mode == Mode.PROD ? "true" : "false");
	...
	public void onConfigurationRead() {
		...
		eTag_ = Play.configuration.getProperty("http.useETag", "true")
	...
	private synchronized void updateRoute_() {
		if (inMemoryCache) {
			...
			Router.addRoute(0, "GET", url + "{key}",
					"greenscript.Service.getInMemoryCache", null, null);
			Logger.debug("route added: %s handled by %s", url + "{key}",
	 ...
	 * Servlet 2.4 does not allow you to get the context path from the
	 * servletcontext...
	 */
	@Override
	...
	public void afterApplicationStart() {
		...
		for (ResourceType type : ResourceType.values()) {
			...
			String s = fetchProp_(
					p,
					String.format("greenscript%s.cache.check",
							type.getExtension()));
			...
			if (-1 != i) {
				...
				JobsPlugin.executor.scheduleWithFixedDelay(j, i, i,
						TimeUnit.SECONDS);
			}
		}
	}
	...
	/*
	 * provided here to avoid compilation error when Rythm Template Engine is
	 * not presented
	 */
	...
	public Template loadTemplate(VirtualFile file) {
		if (rythmPresented_)
			return null; // let rythm to handle it
		if (!file.exists())
			return null;
	...
	private static class ResourceResolver extends Controller {
		public static String def(ResourceType type) {
			...
			return null == template ? null : template.replaceFirst("^views/",
					urlPath).replaceFirst("\\.[\\w]+$", type.getExtension());
		}
	}
	...
	public void beforeActionInvocation(Method actionMethod) {
		/*
		 * Automatically declare js resource e.g
		 * /public/javascripts/Application/index.js
		 */
		...
		/*
		 * Automatically declare js resource e.g.
		 * /public/stylesheets/Application/index.css
		 */
		sess = sessCss_.get();
	...
	public boolean serveStatic(VirtualFile file, Request request,
		...
		if (fn.endsWith(".coffee")
				|| (fn.endsWith(".js") && jsM_.isMinimizeEnabled() && !file
						.relativePath().startsWith(cacheUrlPath_()))) {
			return processStatic_(file, request, response, ResourceType.JS);
		}
		if ((fn.endsWith("css") || fn.endsWith("less"))
				&& cssM_.isMinimizeEnabled()
				&& !file.relativePath().startsWith(cacheUrlPath_())) {
	...
	private boolean processStatic_(VirtualFile file, Request req,
			Response resp, ResourceType type) {
		/*
		 * IRenderSession sess = type == ResourceType.JS ? jsSession() :
		 * cssSession(); if (null != sess && sess.hasDeclared()) { // do not
		 * service static if requesting to minimized files return false; }
		 */
		...
		if (!req.isModified(etag, l)) {
			...
			try {
				...
				resp.status = 200;
				resp.print(content);
				resp.setHeader(Names.LAST_MODIFIED, Utils
						.getHttpDateFormatter().format(new Date(l + 1000)));
				if (eTag_) {
					resp.setHeader(Names.ETAG, etag);
				}
				keepFlash_();
	...
	private HashMap<String, Long> currentConfigFiles() {
		...
		for (VirtualFile vf : Play.roots) {
			...
			if (conf.exists()) {
				files.put(conf.getRealFile().getAbsolutePath(), conf
						.getRealFile().lastModified());
			}
		}
	...
	private boolean filesChanged(HashMap<String, Long> oldFiles,
			HashMap<String, Long> newFiles) {
	...
	public void loadDependencies() {
		...
		for (VirtualFile vf : Play.roots) {
			VirtualFile conf = vf.child("conf/greenscript.conf");
			if (conf.exists()) {
				// info_("loading dependency configuration from %1$s",
				// conf.getRealFile().getAbsolutePath());
				try {
					...
					for (String k : p0.stringPropertyNames()) {
						...
						if (!p.containsKey(k)) {
							...
							v = v + "," + v0;
							// info_("loading duplicate property for %s: %s", k,
							// v);
							p.setProperty(k, v);
						}
					}
					...
					throw new UnexpectedException(
							"error loading conf/greenscript.conf");
				}
			}
		}
	...
	public void InitializeMinimizers() {
		...
		if (p.containsKey("greenscript.coffee.enabled")) {
			System.setProperty("greenscript.coffee.enabled",
					p.getProperty("greenscript.coffee.enabled"));
		}

		if (p.containsKey("greenscript.less.enabled")) {
			System.setProperty("greenscript.less.enabled",
					p.getProperty("greenscript.less.enabled"));
		}
	...
	private IRenderSession newSession_(ResourceType type) {
		return type == ResourceType.JS ? new RenderSession(jsM_, jsD_, type)
			...
			if (k.startsWith(prefix)) {
				...
				if (k0.matches(".*\\s*\\-\\s*$")) {
					...
					for (String s : v.replaceAll("\\s+", "").split(
							IDependenceManager.SEPARATOR)) {
						mergeProperties_(p0, s, k0);
					}
				} else {
					mergeProperties_(p0, k0, v);
				}
				trace_("Found one %1$s dependency: %2$s depends on '%3$s'",
						type, k0, v);
			}
		}
		return p0;
	...
	private void setResourceUrlPath_(String resourceUrlRoot,
			String resourceUrlPath, String ext) {
		...
		if (resourceUrlPath.startsWith("/")) {
			path = resourceUrlPath.startsWith(ctxPath) ? resourceUrlPath
					: ctxPath + resourceUrlPath;
		} else {
	...
	private String resourceUrlRoot_() {
		...
		return urlRoot.startsWith(Play.ctxPath) ? urlRoot : Play.ctxPath
				+ urlRoot;
	}
	...
	private String cacheUrlPath_() {
		...
		if (null == resourceUrlRoot) {
			throw new IllegalStateException(
					"resourceUrlRoot must be initiated first");
		}
		Properties p = Play.configuration;
	...
	private Minimizer initializeMinimizer_(Properties p, ResourceType type) {
		...
		boolean routerMapping = getBooleanProp_(p,
				"greenscript.router.mapping", false);
		...
		if (routerMapping) {
			m.setRouteMapper(new IRouteMapper() {
				...
				public String reverse(String fileName) {
					try {
						String url = Router.reverseWithCheck(fileName,
								Play.getVirtualFile(fileName), false);
		...
		boolean resourcesParameter = getBooleanProp_(p,
				"greenscript.resources.param.enabled", false);
		...
		boolean processInline = getBooleanProp_(p,
				"greenscript.inline.process", false);
		System.setProperty("greenscript.lessCompile.postMerge",
				fetchProp_(p, "greenscript.lessCompile.postMerge"));
	...
	private IBufferLocator bufferLocator_ = new IBufferLocator() {
		...
		@Override
		public BufferResource newBuffer(List<String> resourceNames,
				String extension) {
			...
			String key = UUID.nameUUIDFromBytes(builder.toString().getBytes())
					.toString() + extension;

			Logger.trace(
					"Created key '%s' from resources '%s' and extension '%s'",
					key, builder.toString(), extension);
	...
	private boolean getBooleanProp_(Properties p, String key, boolean def) {
		try {
			String s = p.containsKey(key) ? p.getProperty(key) : defProps_
					.containsKey(key) ? defProps_.getProperty(key) : String
					.valueOf(def);
	...
	public static GreenScriptPlugin getInstance() {
		for (PlayPlugin pp : Play.pluginCollection.getEnabledPlugins()) {
			if (pp instanceof GreenScriptPlugin)
				return (GreenScriptPlugin) pp;
		}
		return null;
	}

	public static void updateMinimizer(boolean minimize, boolean compress,
			boolean cache, boolean inMemoryCache) {
		...
		gs.minConf_.setProperty("greenscript.minimize",
				String.valueOf(minimize));
		gs.minConf_.setProperty("greenscript.compress",
				String.valueOf(compress));
		gs.minConf_.setProperty("greenscript.cache", String.valueOf(cache));
		gs.minConf_.setProperty("greenscript.cache.inmemory",
				String.valueOf(inMemoryCache));
	}
